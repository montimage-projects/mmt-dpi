...
- Test: `bash tests/run_all_tests.sh` — the 12 suites are standalone: no prior build or install needed (most compile straight against `src/`; a couple build the SDK into a throwaway prefix themselves); [docs/AGENT_ENVIRONMENT.md §3](docs/AGENT_ENVIRONMENT.md#3-testing) is authoritative for that count, the runtime band and the exit codes
