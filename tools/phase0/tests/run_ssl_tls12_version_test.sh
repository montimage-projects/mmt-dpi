#!/usr/bin/env bash
#
# run_ssl_tls12_version_test.sh — build the TLS 1.2 record-version check test
# against an ASan/UBSan-instrumented build of the SDK and run it (issue #100).
#
# Steps:
#   1. Build + install the SDK with BUILD=asan into an isolated prefix.
#   2. Compile tools/phase0/tests/ssl_tls12_version_test.c against that
...
#
# Usage: tools/phase0/tests/run_ssl_tls12_version_test.sh
set -euo pipefail

TEST_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${TEST_DIR}/../../.." && pwd)"
PREFIX="${MMT_ASAN_PREFIX:-/tmp/mmt-asan-ssl-tls12}"
BIN="$(mktemp -d)/ssl_tls12_version_test"
...
echo "[2/3] compiling ssl_tls12_version_test (ASan/UBSan)"
...
gcc -g -O1 -fsanitize=address,undefined -fno-sanitize-recover=all \
    -o "${BIN}" "${TEST_DIR}/ssl_tls12_version_test.c" \
...
if [ "${rc}" -eq 0 ]; then
    ...
    echo "✗ TLS 1.2 record-version check test: FAIL (rc=${rc})"
fi
exit "${rc}"
