#!/usr/bin/env bash
#
# run_http2_parser_test.sh — build the HTTP/2 parser hardening test against an
# ASan/UBSan-instrumented build of the SDK and run it (issue #4, H1).
#
# Steps:
#   1. Build + install the SDK with BUILD=asan into an isolated prefix.
#   2. Compile tools/phase0/tests/http2_parser_test.c against that library,
...
#
# Usage: tools/phase0/tests/run_http2_parser_test.sh
set -euo pipefail

TEST_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${TEST_DIR}/../../.." && pwd)"
PREFIX="${MMT_ASAN_PREFIX:-/tmp/mmt-asan-http2}"
BIN="$(mktemp -d)/http2_parser_test"
...
if [ "${rc}" -eq 0 ]; then
    echo "✓ HTTP/2 parser hardening test: PASS"
else
    echo "✗ HTTP/2 parser hardening test: FAIL (rc=${rc})"
fi
exit "${rc}"
