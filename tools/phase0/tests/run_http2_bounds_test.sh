#!/usr/bin/env bash
#
# run_http2_bounds_test.sh — build the HTTP/2 mutation-bounds regression test
...
#   2. Compile tools/phase0/tests/http2_bounds_test.c against that library,
#      itself instrumented with -fsanitize=address,undefined. Internal headers
#      are pulled from the source tree (they are not part of the installed
#      public include set).
#   3. Run it. With -fno-sanitize-recover=all, any out-of-bounds read or write
#      aborts with non-zero status; all assertions must also pass.
#      Exit 0 == clean.
#
# Usage: tools/phase0/tests/run_http2_bounds_test.sh
set -euo pipefail

TEST_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${TEST_DIR}/../../.." && pwd)"
PREFIX="${MMT_ASAN_PREFIX:-$(mktemp -d "${TMPDIR:-/tmp}/asan.XXXXXX")}"
BIN="$(mktemp -d)/http2_bounds_test"
...
if [ "${rc}" -eq 0 ]; then
    echo "✓ HTTP/2 mutation bounds test: PASS"
else
    echo "✗ HTTP/2 mutation bounds test: FAIL (rc=${rc})"
fi
exit "${rc}"
