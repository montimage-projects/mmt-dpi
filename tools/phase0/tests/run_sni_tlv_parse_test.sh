#!/usr/bin/env bash
#
# run_sni_tlv_parse_test.sh — build the SNI extension TLV-parse regression test
# against an ASan/UBSan-instrumented build of the SDK and run it (issue #103).
#
# Steps:
#   1. Build + install the SDK with BUILD=asan into an isolated prefix.
#   2. Compile tools/phase0/tests/sni_tlv_parse_test.c against that library,
...
#
# Usage: tools/phase0/tests/run_sni_tlv_parse_test.sh
set -euo pipefail

TEST_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${TEST_DIR}/../../.." && pwd)"
PREFIX="${MMT_ASAN_PREFIX:-/tmp/mmt-asan-sni-tlv}"
BIN="$(mktemp -d)/sni_tlv_parse_test"
...
echo "[2/3] compiling sni_tlv_parse_test (ASan/UBSan)"
...
gcc -g -O1 -fsanitize=address,undefined -fno-sanitize-recover=all \
    -o "${BIN}" "${TEST_DIR}/sni_tlv_parse_test.c" \
...
echo "[3/3] running sni_tlv_parse_test under ASan/UBSan"
rc=0
...
if [ "${rc}" -eq 0 ]; then
    echo "✓ SNI extension TLV-parse test: PASS"
else
    echo "✗ SNI extension TLV-parse test: FAIL (rc=${rc})"
fi
exit "${rc}"
