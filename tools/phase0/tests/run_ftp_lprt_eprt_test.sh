#!/usr/bin/env bash
#
# run_ftp_lprt_eprt_test.sh — build the FTP LPRT/EPRT IPv6 address-parser
# hardening test against an ASan/UBSan-instrumented build of the SDK and run it
# (issue #8, K4).
#
# Steps:
#   1. Build + install the SDK with BUILD=asan into an isolated prefix.
#   2. Compile tools/phase0/tests/ftp_lprt_eprt_test.c against that library,
#      itself instrumented with -fsanitize=address,undefined.
#   3. Run it. With -fno-sanitize-recover=all, any out-of-bounds read/write
#      aborts with non-zero status; all assertions must also pass. Exit 0 ==
#      clean.
#
# Usage: tools/phase0/tests/run_ftp_lprt_eprt_test.sh
set -euo pipefail

TEST_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${TEST_DIR}/../../.." && pwd)"
PREFIX="${MMT_ASAN_PREFIX:-/tmp/mmt-asan-ftp-lprt-eprt}"
...
if [ "${rc}" -eq 0 ]; then
    echo "✓ FTP LPRT/EPRT hardening test: PASS"
else
    echo "✗ FTP LPRT/EPRT hardening test: FAIL (rc=${rc})"
fi
exit "${rc}"
