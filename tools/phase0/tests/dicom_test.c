...
static void test_pending_210(void) {
    if (!getenv("MMT_PENDING_FIXES")) {
        printf("pending-#210 cases: skipped (set MMT_PENDING_FIXES=1 to arm)\n");
        return;
    }
    printf("pending-#210 cases (must go green only on a tree carrying the #210 fix):\n");
...
int main(void) {
    printf("=== DICOM dissector crafted-input test (issue #215) ===\n");
