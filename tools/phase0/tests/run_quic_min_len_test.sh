#!/usr/bin/env bash
#
# run_quic_min_len_test.sh — build the QUIC minimum-length gating hardening test
# against an ASan/UBSan-instrumented build of the SDK and run it (issue #6, H3).
#
# Steps:
#   1. Build + install the SDK with BUILD=asan into an isolated prefix.
#   2. Compile tools/phase0/tests/quic_min_len_test.c against that library,
...
#
# Usage: tools/phase0/tests/run_quic_min_len_test.sh
set -euo pipefail

TEST_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${TEST_DIR}/../../.." && pwd)"
PREFIX="${MMT_ASAN_PREFIX:-/tmp/mmt-asan-quic}"
BIN="$(mktemp -d)/quic_min_len_test"
...
echo "[2/3] compiling quic_min_len_test (ASan/UBSan)"
...
gcc -g -O1 -fsanitize=address,undefined -fno-sanitize-recover=all \
    -o "${BIN}" "${TEST_DIR}/quic_min_len_test.c" \
...
echo "[3/3] running quic_min_len_test under ASan/UBSan"
...
if [ "${rc}" -eq 0 ]; then
    echo "✓ QUIC minimum-length gating test: PASS"
else
    echo "✗ QUIC minimum-length gating test: FAIL (rc=${rc})"
fi
exit "${rc}"
