...
tests/run_all_tests.sh:183-212	^DEFAULT_SUITES=\(	^\)$	the default suite list lives in DEFAULT_SUITES	sdk_coverage
tests/run_all_tests.sh:225-342	^# --- coverage report	^fi$	--coverage writes an lcov tracefile and a line rate	coverage\.info
...
tests/run_all_tests.sh:164-171	^ *# Harvest this suite	^    fi$	SDK counters are harvested after each suite	sdk-coverage\.sh" harvest
rules/common.mk:548-566	^# BUILD=coverage to compile the SDK	^endif	the BUILD=coverage profile is defined here	\-\-coverage
tests/run_all_tests.sh:344-360	^# --- phase0 harnesses	^fi$	--with-harnesses delegates to the aggregate runner	run_all_harnesses\.sh
...
.github/workflows/c-cpp.yml:231	bash tools/ci/build-docs-site.sh	-	CI builds Jekyll
...
tests/run_all_tests.sh:249-253	json_file=	select\(	coverage restricts emitted records to src	/src/
tests/run_all_tests.sh:310-324	Instrumented files	scope:	coverage summary exposes measured scope	instrumented_sources
