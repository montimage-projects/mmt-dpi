...
- Q: Issue #211 (3.5) — installer elevation was decided by a path literal, the package smoke test globbed a host-shared directory, two prefix validators drifted, the ZIP uninstaller left a directory behind, and the examples branched on uninitialised storage. How are they fixed without breaking the `curl | bash` standalone path?
  A: `install.sh` decides elevation by *writability*: `prefix_writable()` (new shared helper in `dist/ZIP/mmt-install-common.sh`) walks to the nearest existing ancestor and tests `-w`, so `/usr/local/mmt` escalates exactly like `/opt/mmt` while a user-local prefix installs unprivileged — and `post_install` runs `ldconfig` only when `ELEVATED=1` was set by that install (F-BUG-116). A `--prefix <dir>` flag was added as an `MMT_BASE` synonym so the documented verify path works. The duplicate `validate_mmt_base` was removed from `install.sh`: the canonical definition now lives in `dist/ZIP/mmt-install-common.sh` and is sourced by all three installers — early from the checkout when the script runs inside one, or from the verified clone after `clone_repo()` when piped via curl. Because the pinned `v1.8.0` tag ships no `mmt-install-common.sh` at all, `install.sh` keeps a byte-identical fallback of both helpers (the sourced copy wins when present; `tests/installer` diffs the two definitions so they cannot drift — the original defect), and a missing file in the clone is tolerated rather than fatal (F-BUG-121). The validator switched from a metacharacter blacklist to an allowlist (`^/[A-Za-z0-9._/-]+$`), which also rejects space, tab, `( ) { } [ ] # % = : ~ , ^`. `dist/ZIP/install.sh`/`uninstall.sh` became elevation-aware the same way (root required only for unwritable prefixes; `/etc/ld.so.conf.d` + `ldconfig` only when elevated), which makes the install/uninstall round trip testable unprivileged; `uninstall.sh` now also removes `$MMT_EXAMS` and `rmdir`s an emptied `$MMT_BASE`. `tools/ci/build-package.sh` gained `--dry-run`, clears `dist/packages/` before collecting (it is bind-mounted and shared across matrix jobs), and the smoke test installs the tracked `artifacts[]` paths instead of a directory glob (F-BUG-117). Examples: `extract_all.c`/`MAC_extraction.c` initialise `filename`/`type`, reject an unset mode instead of comparing uninitialised storage, and drop the dead array NULL check; `extract_all`'s signal handler now only sets a `volatile sig_atomic_t` flag plus `pcap_breakloop` (printf-based `clean()` in a handler was async-signal-unsafe and could run twice — F-BUG-105), and SIGSEGV/SIGABRT keep their default action. `simple_traffic_reporting.c` dropped its unbounded `proto_hierarchy_names_to_str` for the public bounded `proto_hierarchy_to_str_with_size`, and the `len - 1` last-protocol index (which evaluated to `proto_path[-1]` for an empty hierarchy) is now `get_application_name()` with a NULL guard (F-BUG-104). Verified by the new `tests/installer` suite (60 checks: validator allowlist, dedupe diff, dry-run/elevation plan, ZIP round-trip residue, compiled-example behaviour on `src/examples/google-fr.pcap`).
  Source: `install.sh`, `dist/ZIP/mmt-install-common.sh`, `dist/ZIP/install.sh`, `dist/ZIP/uninstall.sh`, `tools/ci/build-package.sh`, `src/examples/extract_all.c`, `src/examples/MAC_extraction.c`, `src/examples/simple_traffic_reporting.c`, `tests/installer/run_tests.sh`, `src/mmt_core/public_include/data_defs.h:1187-1211`.
