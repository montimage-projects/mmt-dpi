...
| `tests/run_all_tests.sh` | Master test runner and the 29 standalone suites |
...
Expected result: **29/29 suites pass**, total runtime roughly **90–360 s** on
...
success, `1` on any failure. The runner has no `-j` option: the 29 suites run
sequentially. The suite list lives in `DEFAULT_SUITES`
(`tests/run_all_tests.sh:190-220`)
...
`tcp_pending_order`, `sdk_coverage`, `docs_lifecycle`.
...
embedding lifecycle in `docs/USER_GUIDE.md` §3 and compiles and runs the
...
`installed_consumer`, `parser_boundaries`, `sdk_coverage`, `docs_lifecycle`, and the engine
...
- `bash tests/run_all_tests.sh --coverage` — instruments the suites with gcov,
  ...
  (`tests/run_all_tests.sh:233-356`). Requires `gcov` (shipped with
...
- `bash tests/run_all_tests.sh --with-harnesses` — after the suites, runs
  ...
  all harnesses against it (`tests/run_all_tests.sh:358-374`). The arm counts
...
(`tests/run_all_tests.sh:257-261`); its percentage uses the lines in that
...
`summary.json` (`tests/run_all_tests.sh:318-332`). The committed minimum is
...
bash tests/run_all_tests.sh     # 29/29 suites PASSED, exit 0 (90–360 s)
