...
| `tests/run_all_tests.sh` | Master test runner and the 20 standalone suites |
...
Expected result: **20/20 suites pass**, total runtime roughly **60–100 s** on
...
success, `1` on any failure. The runner has no `-j` option: the 20 suites run
sequentially. The suite list lives in `DEFAULT_SUITES`
(`tests/run_all_tests.sh:155-176`)
...
- `bash tests/run_all_tests.sh --coverage` — instruments the suites with gcov,
  ...
  (`tests/run_all_tests.sh:189-298`). Requires `gcov` (shipped with
...
- `bash tests/run_all_tests.sh --with-harnesses` — after the suites, runs
  ...
  all harnesses against it (`tests/run_all_tests.sh:300-316`). The arm counts
