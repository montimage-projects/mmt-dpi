...
| `tests/run_all_tests.sh` | Master test runner and the 26 standalone suites |
...
Expected result: **26/26 suites pass**, total runtime roughly **90–360 s** on
...
success, `1` on any failure. The runner has no `-j` option: the 26 suites run
sequentially. The suite list lives in `DEFAULT_SUITES`
(`tests/run_all_tests.sh:155-182`)
...
- `bash tests/run_all_tests.sh --coverage` — instruments the suites with gcov,
  ...
  (`tests/run_all_tests.sh:195-304`). Requires `gcov` (shipped with
...
- `bash tests/run_all_tests.sh --with-harnesses` — after the suites, runs
  ...
  all harnesses against it (`tests/run_all_tests.sh:306-322`). The arm counts
...
(`tests/run_all_tests.sh:219-223`); its percentage uses the lines in that
...
`summary.json` (`tests/run_all_tests.sh:279-293`). The committed minimum is
...
bash tests/run_all_tests.sh     # 26/26 suites PASSED, exit 0 (90–360 s)
