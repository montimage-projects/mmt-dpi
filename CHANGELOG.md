...
- implement advertised TCP/IP attribute extractions: new `tcp.mss`, `tcp.wscale` and `tcp.sack_permitted` option attributes (appended ids; 7,448 attribute registrations), `tcp.syn_received` now fills its declared 4-byte width, `radius.dns_ipv6` reports the primary 3GPP IPv6 DNS server, and the HTTP attribute-scope lookup reads each attribute's own scope; GRE sequence/loss tracking and the other design-dependent items move to #455 (#331)
