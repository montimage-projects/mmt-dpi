...
int gre_classify_next_proto(ipacket_t * ipacket, unsigned index) {
    ...
    /* Issue #202 (F-BUG-033): the fixed GRE header is 4 bytes — flags word
     * plus protocol — and must be captured before flag bits are consulted. */
    if (offset < 0 || !mmt_have_bytes(ipacket, (size_t) offset, 4))
