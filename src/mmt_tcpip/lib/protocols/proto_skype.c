...
/* Not registered since issue #102 (see inter_proto_classif_list.def): kept as
 * a no-op that only excludes SKYPE, so a re-registration cannot misclassify. */
...
int init_proto_skype_struct() {
    ...
    if (protocol_struct != NULL) {
        ...
        mmt_init_classify_bitmasks(&selection_bitmask, &detection_bitmask,
                &excluded_protocol_bitmask, MMT_SELECTION_BITMASK_PROTOCOL_TCP_OR_UDP_WITH_PAYLOAD,
                PROTO_UNKNOWN, PROTO_SKYPE);
