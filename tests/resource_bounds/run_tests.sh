...
#   no arguments runs every fixture; "ipv6-hash" / "tcp-memory" select one.
set -euo pipefail
...
FIXTURES=( "$@" )
[ "${#FIXTURES[@]}" -eq 0 ] && FIXTURES=(ipv6-hash tcp-memory)
...
for f in "${FIXTURES[@]}"; do
    case "$f" in
        ...
        *) echo "✗ unknown resource_bounds fixture '$f' (known: ipv6-hash, tcp-memory)" >&2
           exit 2 ;;
    esac
done
...
echo "✓ resource bounds tests passed (issues #379, #380)"
